...
import { encode_tapscript } from '@/lib/taproot/lib/encode.js'
...
export function hash_taproot_tx (
  ...
  if (!CONST.VALID_TAPROOT_SIGHASHES.includes(sigflag)) {
  ...
  if (is_anypay) {
    ...
    preimage.push(
      ...
      encode_txout_value(value),             // Commit to the input's prevout value.
...
export function hash_amounts (
  ...
  for (const { value } of prevouts) {
    stack.push(encode_txout_value(value))
  }
  return Buff.join(stack).digest
}
