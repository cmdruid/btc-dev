...
import { encode_script, get_script_asm } from '@/lib/script/index.js'
...
export function hash_segwit_tx (
  ...
  if (!CONST.VALID_SEGWIT_SIGHASHES.includes(flag)) {
  ...
  if (get_script_asm(script).includes('OP_CODESEPARATOR')) {
