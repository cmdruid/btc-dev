...
function verify_input (
  ...
  try {
    ...
    return { index, valid: false, type, error: `Unsupported witness version: ${version}` }
  } catch (err) {
...
function verify_segwit_input (
  ...
  if (type === 'p2wpkh') {
    ...
    if (params.length < 2) {
      return { index, valid: false, type, error: 'Missing pubkey in P2WPKH witness' }
    }
    pubkey = params[1]
    ...
    if (script === null) {
      return { index, valid: false, type, error: 'Missing script in P2WSH witness' }
    }
    hashScript = script
    ...
    if (params.length >= 2) {
      ...
      return { index, valid: false, type, error: 'Missing pubkey in P2WSH witness' }
    }
  } else {
    return { index, valid: false, type, error: `Unexpected segwit type: ${type}` }
  }
  ...
  return { index, valid: isValid, type, error: isValid ? undefined : 'Invalid ECDSA signature' }
}
...
function verify_taproot_input (
  ...
  if (type === 'p2tr') {
    ...
    if (cblock === null || script === null) {
      return { index, valid: false, type, error: 'Missing cblock or script in script-path spend' }
    }
    ...
    if (!pathValid) {
      return { index, valid: false, type, error: 'Control block verification failed' }
    }
    ...
    return { index, valid: false, type, error: `Unexpected taproot type: ${type}` }
  }
  ...
  return { index, valid: isValid, type, error: isValid ? undefined : 'Invalid Schnorr signature' }
}

/**
 * Parse an ECDSA DER-encoded signature and extract sighash flag.
 */
function parse_ecdsa_signature (sigHex : string) : { signature : string, sigflag : number } {
 ...
 */
function parse_schnorr_signature (sigHex : string) : { signature : string, sigflag : number } {
