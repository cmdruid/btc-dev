...
import { keys, signer }    from '@cmdcode/crypto-tools'
import { hash_taproot_tx } from '@cmdcode/tapscript2/sighash'
...
import * as HASH from '@/lib/sighash/lib/taproot.js'
...
export function test_signatures(t : Test) {
  t.test('Test vectors for signature hash construction.', async t => {
    ...
    for (const { given, intermediary, expected } of vectors) {
      ...
      const signature     = sign_taproot_tx(tweakedPrivkey, tx, { sigflag: hashType, txindex : txinIndex, throws : true })
      ...
      const isVerified    = verify_taproot_tx(tx, { sigflag: hashType, txindex: txinIndex, throws : true })
