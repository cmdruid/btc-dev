...
import { get_script_hex } from '@cmdcode/tapscript2/script'
      ...
      if (scripts.length === 0) {
        ...
        for (let i = 0; i < leaves.length; i++) {
          t.test('Testing leaf: ' + leaves[i], t => {
            ...
            const data    = get_script_hex(scripts[i]).hex
            const target  = encode_tapleaf(data, version)
            t.equal(target, leaves[i], 'Leaf hash should match.')
            const { cblock } = get_taproot_ctx({ int_key: internalPubkey, leaves : leafHashes, target,  version })
