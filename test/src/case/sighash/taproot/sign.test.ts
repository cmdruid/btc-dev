  ...
  t.test('Test vectors for signature hash construction.', async t => {
    ...
    for (const { given, intermediary, expected } of vectors) {
      ...
      try {
        ...
        const tweakedpub    = Buff.bytes(schnorr.getPublicKey(Buff.hex(tweakedPrivkey)))
        ...
        const schnorrVerify = schnorr.verify(Buff.hex(signature), Buff.hex(sigHash), tweakedpub)
        ...
        const checkVerify   = schnorr.verify(Buff.hex(witsig), Buff.hex(sigHash), tweakedpub)
        t.true(checkVerify, 'The vector signature should be valid using schnorr.')

        const schnorrSig    = schnorr.sign(actual_hash, Buff.hex(tweakedPrivkey))
