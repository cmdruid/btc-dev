...
import { P2PKH } from '@/dist/address'
...
export default function (t : Test) : void {
  ...
  const hash    = '85be4269276fd45d0b6f7ee963dd073b202d49ed'
  const asm     = [ 'OP_DUP', 'OP_HASH160', hash, 'OP_EQUALVERIFY', 'OP_CHECKSIG' ]
  const hex     = `76a914${hash}88ac`
  const ref     = { asm, hex, key: hash, network: 'testnet', type: 'p2pkh' }
  ...
  t.test('P2PKH unit test', t => {
    ...
    const addr2 = P2PKH.encode(hash, 'regtest')
