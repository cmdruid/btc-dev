...
export default function (t : Test) : void {
  ...
  const keydata  = 'f52611446bdfa1f67da1fb7805dbee74c6d92a54'
  const asm  = [ 'OP_HASH160', keydata, 'OP_EQUAL' ]
  const hex  = `a914${keydata}87`
  const ref  = { asm, hex, keydata, network: 'testnet', type: 'p2sh' }
  ...
  t.test('P2SH unit test', t => {
    ...
    const addr2 = P2SH.encode(keydata, 'regtest')
