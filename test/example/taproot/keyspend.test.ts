...
export async function key_spend (
  ...
  tap.test('Basic spend using key-path.', async t => {
    ...
    try {
      ...
      const { tapkey, taptweak } = get_taproot_ctx({ int_key : pubkey })
      ...
      */
      ...
      const sig     = sign_taproot_tx(tap_sec, txdata, { txindex: 0 })
      ...
      const is_valid = verify_taproot_tx(txdata, { txindex : 0, throws : true })
